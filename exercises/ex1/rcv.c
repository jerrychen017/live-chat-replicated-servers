...
int main(int argc, char** argv) {
  ...
  if (bind(socket_rcv, (struct sockaddr*)&sockaddr_rcv, sizeof(sockaddr_rcv)) <
      0) {
    ...
    if (num > 0) {
      if (FD_ISSET(socket_rcv, &read_mask)) {
        ...
        switch (packet_received.tag) {
          ...
          case 1: {
            unsigned int index =
                convert(packet_received.sequence, start_sequence, start_index);
            ...
            unsigned int cur;
            for (cur = start_sequence; cur < start_sequence + WINDOW_SIZE;
                 cur++) {
              unsigned int actual_index =
                  convert(cur, start_sequence, start_index);
            ...
            for (cur = start_sequence; cur < gap; cur++) {
              int bytes_written =
                  fwrite(window[convert(cur, start_sequence, start_index)], 1,
                         BUF_SIZE, fw);
              // clear the timestamp
                ...
                for (cur = gap; cur < last_received; cur++) {
                  ...
                  if (!occupied[actual_index]) {
                    ...
                    if (!timerisset(timestamps[actual_index])) {
                      ...
                      memcpy(timestamps[acutal_index], &now,
